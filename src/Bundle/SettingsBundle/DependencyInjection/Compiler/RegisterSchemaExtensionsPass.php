...
class RegisterSchemaExtensionsPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds('dag.settings_extension') as $id => $attributes) {
            if (!array_key_exists('namespace', $attributes[0])) {
                throw new InvalidArgumentException(sprintf(
                    'Service "%s" must define the "namespace" attribute on "dag.settings_extension" tags.',
                    $id
                ));
            }
